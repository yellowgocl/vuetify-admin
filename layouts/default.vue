<template>
  <v-app dark>
    <v-navigation-drawer
      ...
      app
    >
    <v-container fluid no-gutters class='flex-column flex-nowrap align-start align-content-start justify-start fill-height pa-0 ma-0'>
      <v-col>
  ...
  mounted(){
    this.$api.post(this.$api.urls.GET_DRAWER_MENU).then(res => {
        ...
        this.items = reduce(res, (r, v, k) => {
            if (isUndefined(v.parentId) || isNull(v.parentId) || v.parentId == res.rootId) {
